...
export default function Dashboard() {
  ...
  const formatSummaryAsText = useCallback(() => {
    ...
    // Convert "SECTION NAME:\ncontent" format to plain text with underlines
    const sectionKeys = ["PORTFOLIO OVERVIEW", "IN PROGRESS", "RECENTLY COMPLETED", "KEY RISKS", "BLOCKERS", "RECOMMENDED ACTIONS"];
    const plainText = aiSummary
      .split("\n")
      .map(line => {
        const trimmed = line.trim();
        const isHeader = sectionKeys.some(k => trimmed === k + ":");
        if (isHeader) {
          const label = trimmed.slice(0, -1); // remove trailing colon
          return "\n" + label + "\n" + "-".repeat(label.length);
        }
