...
def save_page_content(url, content, html, pagination_items=None):
    ...
    with open(txt_path, 'w', encoding='utf-8') as f:
        ...
        if pagination_items:
            f.write(f"\n\n{'='*80}\n")
            f.write(f"PAGINATED ITEMS ({len(pagination_items)} total)\n")
            f.write(f"{'='*80}\n\n")
            for idx, item in enumerate(pagination_items, 1):
                f.write(f"\n--- Item {idx} ---\n")
                f.write(f"Tab: {item.get('tab', 'N/A')}\n")
                content_data = item.get('content', {})
                f.write(f"Title: {content_data.get('title', 'N/A')}\n")
                f.write(f"Description: {content_data.get('description', 'N/A')}\n")
                if content_data.get('links'):
                    f.write(f"Links: {', '.join(content_data['links'])}\n")
